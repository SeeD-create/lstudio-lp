(()=>{
 ...
 function apply(){const s=slides[index];s.querySelectorAll('[data-viz]').forEach(e=>e.classList.toggle('viz-on',step>=+e.dataset.viz&&(!e.hasAttribute('data-viz-until')||step<=+e.dataset.vizUntil)));s.dataset.step=String(step);s.dataset.sceneStep=(s.dataset.sceneTimeline?.split(',')[step]??String(step));s.querySelectorAll('[data-at]').forEach(e=>e.classList.toggle('is-in',step>=+e.dataset.at));s.querySelectorAll('[data-fill]').forEach(e=>{e.classList.toggle('is-filled',step>=+e.dataset.fill);e.classList.toggle('revealed',step>=+e.dataset.fill);});
